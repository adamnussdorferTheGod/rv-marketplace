...
- [ ] **CARD-01**: User sees a headline stat bar with 3-4 key metrics (listing count, median price, trend, avg DOM) above the SRP results
...
- [ ] **CARD-07**: Summary card renders with confidence-gated content — full card at 50+ results, reduced at 10-49, stat bar only at 3-9, hidden at <3
- [ ] **CARD-08**: Every data claim shows a grounding indicator (e.g., "Based on 24 listings matching your search")
...
- [ ] **RESP-01**: Desktop (1024px+): full-width summary card between search bar and first listing row, stat bar + narrative visible by default
...
- [ ] **RESP-04**: User can dismiss the summary card; preference persists in localStorage
- [ ] **RESP-05**: Dismissed state shows subtle "View market insights" link; resets after 30 days
- [ ] **RESP-06**: Assistant responses render with typewriter effect (character-by-character reveal)

### Accessibility

- [ ] **A11Y-01**: Summary card has `role="region"` with `aria-label="Market insights for your search"`
- [ ] **A11Y-02**: Expand/collapse uses `aria-expanded` and `aria-controls`
- [ ] **A11Y-03**: All stats have sr-only context labels ("Median asking price: $62,400" not just "$62,400")
- [ ] **A11Y-04**: Charts have text summary via sr-only or accessible data table fallback
- [ ] **A11Y-05**: Trend arrows use direction + color (not color alone) for accessibility
